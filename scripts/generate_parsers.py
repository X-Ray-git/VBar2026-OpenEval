...
def build():
    ...
    HANDWRITTEN_PARSERS = {'品鉴下半A组和SF', '下半S组纯夸感想', 'X-Ray AF', 'krvspt'}
    ...
    # ── Handwritten parser: krvspt ───────────────────────────────────────────
    # 三列布局：左=初赛上半场(col0-9)，中=初赛下半场(col11-20)，右=决赛(col23-32)
    # 进决赛的曲目两块都有评分，需后处理合并为同一条记录
    code.append('@register_parser("krvspt")')
    code.append('def parse_krvspt(df: pd.DataFrame, reviewer_name: str) -> Tuple[List[ReviewerAssessment], str]:')
    code.append('    assessments = []')
    code.append('    decl = extract_declaration(df, 2)')
    code.append('    import re as _krre')
    code.append('    _COLOR_GREEN = ("FF92D050", "FF00FF00", "FFC3EAD5", "FF277C4F", "FF184E32")')
    code.append('    _COLOR_RED   = ("FFFF0000", "FFE6615D", "FFDE3C36", "FFE74025", "FFFF9C99")')
    code.append('    _COLOR_YELLOW= ("FFFFC000", "FFFFFF00", "FFF88825", "FFF5C401", "FFF4C243", "FFA38200")')
    code.append('    _COLOR_BLUE  = ("FF00A3F5", "FF2972F4", "FFBDD7EE", "FF1450B8", "FF1274A5")')
    code.append('    def _krcv(row, c, r, sheet):')
    code.append('        if c >= len(row): return ""')
    code.append('        v = clean_str(row[c])')
    code.append('        if not v: return ""')
    code.append('        color = get_color(sheet, r, c)')
    code.append('        if color:')
    code.append('            cu = color.upper()')
    code.append('            if cu in _COLOR_GREEN: v += "【绿色底色：给票/推荐】"')
    code.append('            elif cu in _COLOR_RED: v += "【红色底色：多倍票/重点】"')
    code.append('            elif cu in _COLOR_YELLOW: v += "【黄色底色：斟酌/待定】"')
    code.append('            elif cu in _COLOR_BLUE: v += "【蓝色表态】"')
    code.append('        return v')
    code.append('    def _krid(sid):')
    code.append('        if not sid or len(sid) < 2: return False')
    code.append('        if _krre.fullmatch(r"[A-Za-z]{2}[12]", sid): return False')
    code.append('        for kw in ("上半","下半","待定","评委","编号","（","("):')
    code.append('            if kw in sid: return False')
    code.append('        return True')
    code.append('    _DIMNAMES = ["软件操作使用","艺术表达技巧","歌声解析能力","发展潜力"]')
    # 三个 Block 的列定义
    code.append('    _BLOCKS = [')
    code.append('        (0, 1, 2, 6, 7, 8, 9),')
    code.append('        (11, 12, 13, 17, 18, 20, 19),')
    code.append('        (23, 24, 25, 29, 30, 32, 31),')
    code.append('    ]')
    code.append('    _BLOCK_LABELS = ["初赛", "初赛", "决赛"]')
    code.append('    _ghost_krvs = {"问号真来啊","我认为可行","←那很敢吃了","114514","111","好像真是一样的","????","←我觉得真应该在等级里加个问号（","支持↑","SC开篇就是新年财神到，大家新年发大财呀！","这首还不错，人声很有感情","很有意思"}')
    # 主循环
    code.append('    for idx in range(3, len(df)):')
    code.append('        row = df.iloc[idx].values')
    code.append('        for bi, (id_c, name_c, dim_base, overall_c, comment_c, audience_c, vote_c) in enumerate(_BLOCKS):')
    code.append('            if id_c >= len(row): continue')
    code.append('            song_id = clean_str(row[id_c]).replace(" ","")')
    code.append('            if not _krid(song_id): continue')
    code.append('            song_name = clean_str(row[name_c]) if name_c < len(row) else ""')
    code.append('            dimensions = {}')
    code.append('            for i, dn in enumerate(_DIMNAMES):')
    code.append('                v = _krcv(row, dim_base+i, idx, reviewer_name)')
    code.append('                if v: dimensions[dn] = v')
    code.append('            overall = _krcv(row, overall_c, idx, reviewer_name)')
    code.append('            vote = _krcv(row, vote_c, idx, reviewer_name)')
    code.append('            if vote: dimensions["投票"] = vote')
    code.append('            comment_v = _krcv(row, comment_c, idx, reviewer_name)')
    code.append('            comments_list = []')
    code.append('            if comment_v: comments_list.append("[瑞平评语]\\n" + comment_v)')
    code.append('            audience_v = _krcv(row, audience_c, idx, reviewer_name)')
    code.append('            audience = {"观众留言": audience_v} if audience_v else {}')
    code.append('            has_content = bool(dimensions or overall or comments_list or audience)')
    code.append('            if not has_content: continue')
    code.append('            _dv = "".join(dimensions.values()).upper()')
    code.append('            if "A" in _dv and "S" in _dv and overall.upper() in ("?","？"): continue')
    code.append('            if _dv == "AASS": continue')
    code.append('            _has_real = False')
    code.append('            for _t in comments_list + [audience_v if audience_v else ""]:')
    code.append('                if not _t: continue')
    code.append('                _c = _t.split("]\\n")[-1] if "]\\n" in _t else _t')
    code.append('                if _c.strip() not in _ghost_krvs: _has_real = True')
    code.append('            if (comments_list or audience_v) and not _has_real: continue')
    code.append('            assessments.append(ReviewerAssessment(')
    code.append('                reviewer_name=reviewer_name,')
    code.append('                song_id=song_id,')
    code.append('                song_name=clean_newline(song_name),')
    ...
    code.append('                comments=clean_newline("\\n\\n".join(comments_list)),')
    code.append('                audience_comments=audience,')
    code.append('                extra_fields={"_block": str(bi)}')
    code.append('            ))')
    # 后处理合并：同一 song_id 出现两次（初赛 bi=0/1 + 决赛 bi=2）→ 合并
    code.append('    merged = {}')
    code.append('    seen = {}')
    code.append('    for a in assessments:')
    code.append('        is_finals = a.extra_fields.get("_block") == "2"')
    code.append('        if a.song_id not in seen:')
    code.append('            seen[a.song_id] = a')
    code.append('        else:')
    code.append('            prel = seen[a.song_id]')
    code.append('            fin  = a if is_finals else prel')
    code.append('            pr   = prel if not is_finals else a')
    code.append('            prel_dims = {"初赛_" + k: v for k, v in pr.dimension_scores.items()}')
    code.append('            fin_dims  = {"决赛_" + k: v for k, v in fin.dimension_scores.items()}')
    code.append('            p_overall = "[初赛] " + pr.overall_score if pr.overall_score else ""')
    code.append('            f_overall = "[决赛] " + fin.overall_score if fin.overall_score else ""')
    code.append('            merged_overall = " / ".join(x for x in [p_overall, f_overall] if x)')
    code.append('            p_cmt = "[初赛评语]\\n" + pr.comments if pr.comments else ""')
    code.append('            f_cmt = "[决赛评语]\\n" + fin.comments if fin.comments else ""')
    code.append('            merged_comments = "\\n\\n".join(x for x in [p_cmt, f_cmt] if x)')
    code.append('            merged_aud = {**pr.audience_comments}')
    code.append('            for k, v in fin.audience_comments.items():')
    code.append('                merged_aud["决赛_" + k if k in merged_aud else k] = v')
    code.append('            merged[a.song_id] = ReviewerAssessment(')
    code.append('                reviewer_name=reviewer_name,')
    code.append('                song_id=pr.song_id,')
    code.append('                song_name=pr.song_name,')
    code.append('                dimension_scores={**prel_dims, **fin_dims},')
    code.append('                overall_score=merged_overall,')
    code.append('                comments=merged_comments,')
    code.append('                audience_comments=merged_aud,')
    code.append('                extra_fields={}')
    code.append('            )')
    code.append('    final = []')
    code.append('    emitted = set()')
    code.append('    for a in assessments:')
    code.append('        if a.song_id in merged:')
    code.append('            if a.song_id not in emitted:')
    code.append('                final.append(merged[a.song_id])')
    code.append('                emitted.add(a.song_id)')
    code.append('        else:')
    code.append('            a.extra_fields.pop("_block", None)')
    code.append('            final.append(a)')
    code.append('    return final, decl')
    code.append('')
